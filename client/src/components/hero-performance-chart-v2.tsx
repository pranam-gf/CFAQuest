...
export function HeroPerformanceChartV2() {
  ...
  const margin = isMobile 
    ? { top: 80, right: 20, bottom: 120, left: 50 }
    : { top: 100, right: 100, bottom: 150, left: 80 };
  ...
  if (chartData.length === 0) {
    return <div className={`w-full ${isMobile ? 'h-[500px]' : 'h-[700px]'} flex items-center justify-center text-gray-500 dark:text-gray-400`}>Loading...</div>;
  }
  ...
  if (isTransitioning) {
    return (
      <div className="relative w-full overflow-x-auto">
        ...
        <div className={`${isMobile ? 'w-full overflow-x-auto' : 'min-w-fit'} flex justify-center`}>
          <svg width={width} height={height} className="rounded-xl bg-transparent">
            <g transform={`translate(${margin.left}, ${margin.top})`}>
              ...
              {Array.from({ length: 5 }).map((_, i) => {
                ...
                return (
                  <g key={i} className="text-gray-400 dark:text-gray-600">
                    ...
                    <text x={-10} y={y + 4} textAnchor="end" className={`${isMobile ? 'text-[10px]' : 'text-xs'} fill-current`}>{
              ...
              <text
                ...
                y={isMobile ? -35 : -50}
                textAnchor="middle"
                transform={`rotate(-90, -${chartHeight / 2}, ${isMobile ? -35 : -50})`}
                className={`${isMobile ? 'text-sm' : 'text-base'} font-semibold fill-gray-700 dark:fill-gray-300`}
              >
        ...
        <svg width={width} height={height} className="rounded-xl bg-transparent">
          <g transform={`translate(${margin.left}, ${margin.top})`}>
            ...
            {sortedData.map((p, index) => {
              ...
              return (
                <motion.text
                  ...
                  x={xPos}
                  y={chartHeight + (isMobile ? 25 : 35)}
                  textAnchor="end"
                  className={`${isMobile ? 'text-xs' : 'text-sm'} font-medium fill-gray-700 dark:fill-gray-300`}
                  transform={`rotate(-35, ${xPos}, ${chartHeight + (isMobile ? 25 : 35)})`}
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  transition={{ duration: 0.4, delay: index * 0.08 + 0.2 }}
                >
                  {isMobile ? getShortDisplayName(p.model) : getDisplayName(p.model)}
                </motion.text>
              );
            })}
            ...
            {sortedData.map((p, index) => {
              ...
              const displayScore = activeMetric === 'mcq' 
                ? isMobile ? score.toFixed(1) : `${score.toFixed(1)}%`
                : score.toFixed(3);
              
              const slotWidth = chartWidth / sortedData.length;
              const barWidth = slotWidth * (isMobile ? 0.5 : 0.6);
              ...
              return (
                ...
                >
                          ...
                          <div className="space-y-1 text-xs">
                            <div className="flex justify-between gap-4">
                              ...
                              <span className="font-medium">
                                {activeMetric === 'mcq' ? `${score.toFixed(1)}%` : score.toFixed(3)}
                              </span>
                            </div>
                            <div className="flex justify-between gap-4">
                  ...
                  <motion.text 
                    x={slotWidth / 2}
                    y={barTopY + (activeMetric === 'mcq' ? (isMobile ? 20 : 30) : (isMobile ? 15 : 22))}
                    textAnchor="middle"
                    className={`${isMobile ? 'text-xs' : 'text-sm'} font-bold fill-black/70 dark:fill-white/80 pointer-events-none`}
                  ...
                  >
                    <foreignObject
                      x={slotWidth / 2 - (isMobile ? 12 : 16)}
                      y={barTopY - (isMobile ? 30 : 40)}
                      width={isMobile ? "24" : "32"}
                      height={isMobile ? "24" : "32"}
                    >
                      <div className={`${isMobile ? 'w-6 h-6' : 'w-8 h-8'} flex items-center justify-center`}>
                        <ProviderLogo modelName={p.model} className={`${isMobile ? 'w-4 h-4' : 'w-6 h-6'} ${needsInvert ? 'dark:invert' : ''}`} />
                      </div>
                    </foreignObject>
                  </motion.g>
